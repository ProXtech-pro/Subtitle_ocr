...
from subtitle_ocr.types import Paths, Tooling, OCRSettings, RunResult
from subtitle_ocr.settings_store import GUISettings, load_settings, save_settings, settings_path
...
class App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.input_dir = tk.StringVar(value=persisted.input_dir or str(defaults.input_dir))
        self.output_dir = tk.StringVar(value=persisted.output_dir or str(defaults.output_dir))
        self.log_dir = tk.StringVar(value=persisted.log_dir or str(defaults.log_dir))

        self.pgsrip_lang = tk.StringVar(value=persisted.pgsrip_lang or defaults.pgsrip_lang)
        self.tess_lang = tk.StringVar(value=persisted.tess_lang or defaults.tess_lang)
        self.tessdata_prefix = tk.StringVar(value=persisted.tessdata_prefix or str(defaults.tessdata_prefix or ""))
        self.mkv_dir = tk.StringVar(value=persisted.mkvtoolnix_dir or str(defaults.mkvtoolnix_dir or ""))
        self.tess_exe = tk.StringVar(value=persisted.tess_exe or str(defaults.tesseract_exe))
    ...
    def _build(self) -> None:
        ...
        ttk.Entry(dir_frame, textvariable=self.input_dir).grid(row=0, column=1, sticky="ew", padx=6)
        ttk.Button(dir_frame, text="Browse", command=self._browse_input).grid(row=0, column=2, padx=4)
        ttk.Button(dir_frame, text="Scan", command=self.scan).grid(row=0, column=3, padx=4)

        ttk.Label(dir_frame, text="Output:").grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(dir_frame, textvariable=self.output_dir).grid(row=1, column=1, sticky="ew", padx=6, pady=(6, 0))
        ttk.Button(dir_frame, text="Browse", command=self._browse_output).grid(row=1, column=2, padx=4, pady=(6, 0))
        ttk.Button(dir_frame, text="Output=Input", command=lambda: self.output_dir.set(self.input_dir.get())).grid(row=1, column=3, padx=4, pady=(6, 0))

        ttk.Label(dir_frame, text="Logs:").grid(row=2, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(dir_frame, textvariable=self.log_dir).grid(row=2, column=1, sticky="ew", padx=6, pady=(6, 0))
        ttk.Button(dir_frame, text="Browse", command=self._browse_logs).grid(row=2, column=2, padx=4, pady=(6, 0))
        ...
        self.listbox = tk.Listbox(files_frame, selectmode=tk.EXTENDED, font=("Consolas", 10))
        ...
        ttk.Button(btns, text="Select all", command=lambda: self.listbox.selection_set(0, tk.END), width=18).grid(row=0, column=0, pady=2)
        ttk.Button(btns, text="Deselect", command=lambda: self.listbox.selection_clear(0, tk.END), width=18).grid(row=1, column=0, pady=2)
        ttk.Button(btns, text="Remove selected", command=self.remove_selected, width=18).grid(row=2, column=0, pady=(10, 2))
        ttk.Button(btns, text="Clear", command=self.clear_files, width=18).grid(row=3, column=0, pady=2)
        ...
        ttk.Entry(opt, textvariable=self.pgsrip_lang, width=10).grid(row=0, column=1, sticky="w", padx=6)
        ttk.Label(opt, text="TESS_LANG:").grid(row=0, column=2, sticky="w")
        ttk.Entry(opt, textvariable=self.tess_lang, width=10).grid(row=0, column=3, sticky="w", padx=6)

        ttk.Label(opt, text="Tags:").grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(opt, textvariable=self.tags_var).grid(row=1, column=1, columnspan=3, sticky="ew", padx=6, pady=(6, 0))

        ttk.Label(opt, text="Max workers:").grid(row=2, column=0, sticky="w", pady=(6, 0))
        ttk.Spinbox(opt, from_=1, to=64, textvariable=self.max_workers, width=8).grid(row=2, column=1, sticky="w", padx=6, pady=(6, 0))
        ttk.Checkbutton(opt, text="Force overwrite", variable=self.force).grid(row=2, column=2, sticky="w", pady=(6, 0))
        ttk.Checkbutton(opt, text="Rip all tracks", variable=self.rip_all).grid(row=2, column=3, sticky="w", pady=(6, 0))

        ttk.Checkbutton(opt, text="Debug verbose", variable=self.debug_verbose).grid(row=3, column=0, sticky="w", pady=(6, 0))
        ttk.Checkbutton(opt, text="Keep temp files", variable=self.keep_temp).grid(row=3, column=1, sticky="w", pady=(6, 0))

        ttk.Label(opt, text="tesseract.exe:").grid(row=4, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(opt, textvariable=self.tess_exe).grid(row=4, column=1, columnspan=3, sticky="ew", padx=6, pady=(6, 0))

        ttk.Label(opt, text="TESSDATA_PREFIX:").grid(row=5, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(opt, textvariable=self.tessdata_prefix).grid(row=5, column=1, columnspan=3, sticky="ew", padx=6, pady=(6, 0))

        ttk.Label(opt, text="MKVToolNix dir:").grid(row=6, column=0, sticky="w", pady=(6, 0))
        ttk.Entry(opt, textvariable=self.mkv_dir).grid(row=6, column=1, columnspan=3, sticky="ew", padx=6, pady=(6, 0))
        ...
        ttk.Label(footer, text=f"Settings: {settings_path()}").grid(row=0, column=0, sticky="w")
    ...
    def _current_paths_tooling_settings(self):
        ...
        tooling = Tooling(
            ...
            mkvtoolnix_dir=Path(self.mkv_dir.get()).expanduser() if self.mkv_dir.get().strip() else None,
            tessdata_prefix=Path(self.tessdata_prefix.get()).expanduser() if self.tessdata_prefix.get().strip() else None,
        )
    ...
    def _validate_mkvtoolnix(self, mkv_dir: Optional[Path]) -> Optional[str]:
        if not mkv_dir:
            return (
                "MKVTOOLNIX_DIR is empty. Expected a folder that contains mkvmerge.exe / mkvextract.exe.\n"
    ...
    def _validate_tessdata(self, tessdata: Optional[Path], tess_lang: str) -> Optional[str]:
        if not tessdata:
            return (
                "TESSDATA_PREFIX is empty.\n"
                "Set TESSDATA_PREFIX=tessdata_best (project-local) and place traineddata files there."
            )
        if not tessdata.exists():
            return f"TESSDATA_PREFIX folder does not exist: {tessdata}"
        required = TESS_TRAINEDDATA_MAP.get(tess_lang.strip(), f"{tess_lang.strip()}.traineddata")
        if not (tessdata / required).exists():
            return (
                f"Missing traineddata for TESS_LANG='{tess_lang}': expected {required} in\n"
                f"{tessdata}\n"
                "Place the traineddata file(s) into the project-local tessdata_best folder."
            )
        return None
    ...
    def check_setup(self) -> None:
        ...
        ok, tmsg = check_tesseract(tooling.tesseract_exe, settings.tess_lang, log=self._log)
        problems = []
        if not ok:
        ...
        tess_problem = self._validate_tessdata(tooling.tessdata_prefix, settings.tess_lang)
        if tess_problem:
    ...
    def _export_json(self, out: Path) -> None:
        payload = {"generated_at": datetime.now().isoformat(timespec="seconds"), "results": self._run_results}
        out.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    def _export_csv(self, out: Path) -> None:
        fieldnames = [
            "video", "success", "message", "output_srt",
            "status", "size", "lines", "subtitles", "time_sequences",
            "empty_lines", "avg_subtitle_length", "duration_seconds",
        ]
    ...
    def start(self) -> None:
        ...
        if not self.all_files:
            messagebox.showwarning("No files", "No video files to process. Click Scan first.")
            return
        ...
        def worker():
            ...
            for i, vp in enumerate(self.all_files, start=1):
                ...
                res = process_video(vp, paths=paths, tooling=tooling, settings=settings, log=self._log)
